...
function mango(quantity, price){
  let mangoPayCount = Math.ceil(quantity * 2/3)
  return mangoPayCount * price
}
...
function calculateTip(amount, rating) {
  if (typeof amount !=='number') return 'Rating not recognised'
  if (typeof rating !=='string') return 'Rating not recognised'
  ...
  if (rating in ratingKey) {
    return Math.ceil(amount * ratingKey[rating])
  } else {
